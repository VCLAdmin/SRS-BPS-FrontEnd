...
@Component({
  selector: 'app-frame-combination-table',
  templateUrl: './frame-combination-table.component.html',
  styleUrls: ['./frame-combination-table.component.css']
})
export class FrameCombinationTableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.cpService.obsPopout.pipe(takeUntil(this.destroy$)).subscribe(
      response => {
        if (this.unified3DModel !== undefined && response.panelsModule === PanelsModule.FrameCombination) {
   ...
   getTableData(data: any[]) {
    ...
    data.forEach((article, index) => {
        this.data.push({
          ...
          inside: article.inside.toString(),
          outside: article.outside.toString(),
   ...
   */
  getSelectedIndex() {
    let savedData = this.umService.get_FrameCombination('');
  ...
  onClose(): void {
    ...
    this.searchValue = '';
    this.search();
    if (this.isPopoutOpened) this.cpService.setPopout(false, PanelsModule.FrameCombination);
  }
  ...
  /**
   * When the user clicks on the confirm button or double clicks on a row, the information of the row selected is sent to the unified model service to update the frame combination property.
   * The table is also closed.
   */
  onConfirm() {
    //this.umService.set_FrameCombination(this.listOfDisplayData[this.selectedIndex]);
    this.onClose();
  }
  //#endregion
}