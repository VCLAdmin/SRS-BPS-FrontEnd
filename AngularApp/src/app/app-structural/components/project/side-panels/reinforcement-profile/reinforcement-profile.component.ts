...
@Component({
  selector: 'app-reinforcement-profile',
  templateUrl: './reinforcement-profile.component.html',
  styleUrls: ['./reinforcement-profile.component.css']
})
export class ReinforcementProfileComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.cpService.obsPopout.pipe(takeUntil(this.destroy$)).subscribe(
      response => {
        if (this.unified3DModel !== undefined && response.panelsModule === PanelsModule.ReinforcementSliding) {
   ...
   */
   getTableData(data: any[]) {
    this.data = [];
    // data.filter(article => article.System == this.systemSelected.Value && article.SRS == 1 &&
    //   (article.ProfileType == "Steel Tube" ||
    //   article.ProfileType == "Structural Profile" ||
    //   (article.ProfileType == "Double-Vent Profile" && (this.unified3DModel.ModelInput.Geometry.OperabilitySystems[0].VentOperableType.includes("2D") || this.unified3DModel.ModelInput.Geometry.OperabilitySystems[0].VentOperableType.includes("3F")))
    //   )).forEach((article, index) => {
    data.filter(article => article.System == this.systemSelected.Value && article.SRS == 1 && article.ProfileType == "Steel Tube").forEach((article, index) => {
      this.data.push({
        id: '' + index,
   ...
   */
   getSelectedIndexAfterSearch() {
    ...
    if (savedData) {
      this.selectedIndex = this.listOfDisplayData.map(article => article.description).indexOf(savedData.SteelTubeArticleName);
    }
    setTimeout(() => {
   ...
   */
   onClose(): void {
    ...
    this.searchValue = '';
    this.search();
    if (this.isPopoutOpened) this.cpService.setPopout(false, PanelsModule.ReinforcementSliding);
  }
  ...
  /**
   * When the user clicks on the confirm button or double clicks on a row, the information of the row selected is sent to the unified model service to update the reinforcement profile property.
   * The table is also closed.
   */
   onConfirm() {
    this.umService.set_ReinforcementFrame(this.listOfDisplayData[this.selectedIndex]);
    this.onClose();
  }
  //#endregion
}
