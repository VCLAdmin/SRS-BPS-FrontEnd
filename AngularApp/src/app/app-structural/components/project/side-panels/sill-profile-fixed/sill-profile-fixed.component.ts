...
@Component({
  selector: 'app-sill-profile-fixed',
  templateUrl: './sill-profile-fixed.component.html',
  styleUrls: ['./sill-profile-fixed.component.css']
})
export class SillProfileFixedComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
     ...
     */
     this.cpService.obsPopout.pipe(takeUntil(this.destroy$)).subscribe(
      response => {
        if (this.unified3DModel !== undefined && response.panelsModule === PanelsModule.SillProfileFixed) {
  ...
  onPopoutOpened() {
    if (!this.systemSelected) {
      this.systemSelected = this.cpService.currentSystem.Images;
    }
   ...
   */
   fillData(dataList: any) {
    ...
    if (this.permService.checkPermission(Feature.ADS_75)){ //DoorLeaf feature has been added for SRS
        data = data.filter(x => x.Type == "SideLightSill");
      }
   ...
   */
   getSelectedIndex() {
    let savedData = this.umService.obj_SillProfileFixed();
    this.selectedIndex = 0;
    if (savedData) {
      this.selectedIndex = this.listOfDisplayData.map(article => article.description).indexOf(savedData.DoorSidelightSillArticleName);
    }
    if (this.selectedIndex < 0)
      this.selectedIndex = 0;

    let ds = this.unified3DModel.ModelInput.Geometry.DoorSystems;
    if (ds && ds.filter(glass => glass.DoorSidelightSillArticleName === null || glass.DoorSidelightSillArticleName === undefined).length > 0) {
      this.onConfirm();
    }
  }
  //#endregion
   ...
   */
   onClose(): void {
    ...
    this.searchValue = '';
    this.search();
    if (this.isPopoutOpened) this.cpService.setPopout(false, PanelsModule.SillProfileFixed);
  }
  ...
  /**
   * When the user clicks on the confirm button or double clicks on a row, the information of the row selected is sent to the unified model service to update the still profile fixed property.
   * The table is also closed.
   */
   onConfirm() {
    this.umService.set_SillProfileFixed(this.listOfDisplayData[this.selectedIndex]);
    this.onClose();
  }
  //#endregion
}