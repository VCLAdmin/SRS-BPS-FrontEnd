...
@Component({
  selector: 'app-structural-profile-table',
  templateUrl: './structural-profile-table.component.html',
  styleUrls: ['./structural-profile-table.component.css']
})
export class StructuralProfileTableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.cpService.obsPopout.pipe(takeUntil(this.destroy$)).subscribe(
      response => {
        if (this.unified3DModel !== undefined && response.panelsModule === PanelsModule.StrucuralSliding) {
  ...
  getTableData(data: any[]) {
    this.data = [];
    data.filter(article => article.System == this.systemSelected.Value && article.SRS == 1 && article.ProfileType == "Structural Profile").forEach((article, index) => {
      this.data.push({
        id: '' + index,
   ...
   */
   getSelectedIndexAfterSearch() {
    ...
    if (savedData) {
      this.selectedIndex = this.listOfDisplayData.map(article => article.description).indexOf(savedData.StructuralProfileArticleName);
    }
    setTimeout(() => {
   ...
   */
   onClose(): void {
    ...
    this.searchValue = '';
    this.search();
    if (this.isPopoutOpened) this.cpService.setPopout(false, PanelsModule.StrucuralSliding);
  }
  ...
  /**
   * When the user clicks on the confirm button or double clicks on a row, the information of the row selected is sent to the unified model service to update the structural profile property.
   * The table is also closed.
   */
   onConfirm() {
    this.umService.set_structuralProfile(this.listOfDisplayData[this.selectedIndex]);
    this.onClose();
  }
  //#endregion
}
